    ...
    public function __construct($username){
        ...
        if (mysqli_connect_error()){
          die('Connect Error ('. mysqli_connect_errno() .') '
            ...
            echo "<br>";
            if($data_list["role"]== "Main Clerk"){
                require_once '../Clerks/MainClerk.php';
                $this->Admin = new MainClerk($data_list["uname1"],$data_list["email"],$data_list["upswd1"],$data_list["ID"],$data_list["role"]);   
            }elseif($data_list["role"]== "Hospital Clerk"){
                require_once '../Clerks/HospitalClerk.php';
                $this->Admin = new HospitalClerk($data_list["uname1"],$data_list["email"],$data_list["upswd1"],$data_list["ID"],$data_list["role"]);
            }
            echo "clerk created";
            $conn->close();
        }
